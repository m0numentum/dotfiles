...
function wireless_interface_state() {
	...
	[[ ${#WIRELESS_INTERFACES[@]} -eq "0" ]] || {
		...
		{ [[ "$WIFI_CON_STATE" == "indisponible" ]] && WIFI_LIST="***Wi-Fi Disabled***" && WIFI_SWITCH="~Wi-Fi On" && OPTIONS="${WIFI_LIST}\n${WIFI_SWITCH}\n~Scan\n"; } || { [[ "$WIFI_CON_STATE" =~ "connecté" ]] && {
...
function ethernet_interface_state() {
	[[ ${#WIRED_INTERFACES[@]} -eq "0" ]] || {
		...
		{ [[ "$WIRED_CON_STATE" == "déconnecté" ]] && WIRED_SWITCH="~Eth On"; } || { [[ "$WIRED_CON_STATE" == "connecté" ]] && WIRED_SWITCH="~Eth Off"; } || { [[ "$WIRED_CON_STATE" == "indisponible" ]] && WIRED_SWITCH="***Wired Unavailable***"; } || { [[ "$WIRED_CON_STATE" == "connexion" ]] && WIRED_SWITCH="***Wired Initializing***"; }
...
function rofi_menu() {
	...
	{ [[ "$WIRED_CON_STATE" == "connecté" ]] && PROMPT="${WIRED_INTERFACES_PRODUCT}[$WIRED_INTERFACES]"; } || PROMPT="${WIRELESS_INTERFACES_PRODUCT[WLAN_INT]}[${WIRELESS_INTERFACES[WLAN_INT]}]"
...
function rofi_cmd() {
	...
	rofi -m primary -dmenu -i -location "$LOCATION" -yoffset "$Y_AXIS" -xoffset "$X_AXIS" $3 -theme "$RASI_DIR" -theme-str 'window{width: '"$WIDTH"'em;}textbox-prompt-colon{str:"'"$PROMPT"':";}'"$4"''
}
...
function scan() {
	[[ "$WIFI_CON_STATE" =~ "indisponible" ]] && change_wifi_state "Wi-Fi" "Enabling Wi-Fi connection" "on" && sleep 2
